...
from model.ddpg_wraper import DdpgModelWrapper
import numpy as np
...
class DDPGTrainer():
    ...
    def model_wrapper_init(self, model_arch_list, optimizer, loss_function, obs_shape_dict, 
        act_n, learning_rate, gamma, weight_decay, tau, device, model_save_dir):
        self.model_wrapper = DdpgModelWrapper(model_arch_list, optimizer, loss_function, obs_shape_dict, 
                                                    learning_rate, gamma, weight_decay, tau, device, model_save_dir)
    ...
    def update_all(self, total_step: int, need_train: bool, need_render = bool):
        ...
        with tqdm(total = total_step) as t:
            ...
            for i in range(1, total_step):
                actions = self.__choose_action(states=states, need_train=need_train)
    ...
    def __choose_action(self, states:dict, need_train: bool, epsilon = 0.1):

        states_dict = self.model_wrapper.numpy2net_tensor(states)
        actions = self.model_wrapper.actor(states_dict).detach().cpu().numpy()
        actions += need_train*max(epsilon, 0)*self.model_wrapper.random_process.sample()
        actions = np.clip(actions, -1, 1)
        # squeeze the actions twice
