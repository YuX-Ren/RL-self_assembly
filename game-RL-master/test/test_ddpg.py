...
from config.ddpg_asm_train_setting import *
from dqn.ddpg_trainer import DDPGTrainer 

if __name__ == '__main__':

    ddpg_trainer = DDPGTrainer(start_eps_rate= START_EPS_RATE,
    ...
    ddpg_trainer.vec_game_init(nums= ENVS_NUM,
                            game_class=game_class,
                            monitor_file_dir=MONITOR_DIR,
                            obs_shape_dict= OBS_SHAPE)
    ddpg_trainer.model_wrapper_init(model_arch_list= MODEL_CLASS,
                                optimizer= OPTIMIZER,
    ...
    ddpg_trainer.replay_buffer_init(buffer_arch= BUFFER_ARCH,
                                buffer_size= BUFFER_SIZE,
                                obs_shape_dict= OBS_SHAPE)
    ddpg_trainer.update_all(total_step=TIMESTEP, need_train= NEED_TRAIN, need_render= NEED_RENDER)
